...
function renderPreview() {
  ...
  if (hkInfo) {
    ...
    if (globalKeys + pageKeys > 0) {
      ...
      (dashboardData.hotkeys || []).forEach(h => metaHtml += `• [Global] <b>${h.key}</b> ${describe(h)}<br>`);
      (dashboardData.longHotkeys || []).forEach(h => metaHtml += `• [Global, long] <b>${h.key}</b> ${describe(h)}<br>`);
      (page.hotkeys || []).forEach(h => metaHtml += `• [Page] <b>${h.key}</b> ${describe(h)}<br>`);
      (page.longHotkeys || []).forEach(h => metaHtml += `• [Page, long] <b>${h.key}</b> ${describe(h)}<br>`);
      metaHtml += `</div>`;
    }
