...
Ten common card types have full dedicated forms (`bubble_light`, `light`, `switch`, `cover`, `fan`, `climate`, `source_select`, `button_grid`, `scene_grid`, `apple_tv_remote`, `tv_remote`). The remaining types (`clock_weather`, `custom_ir`, `media_player`, `speaker_group`, `monitor`, `picture_elements`, `plex`, `row`, `vacuum`, or any custom type) use a raw JSON options field — check the corresponding `CardRenderer`'s Kotlin file in `src/main/java/com/custom/astrion/cards/impl/` for its exact fields.
